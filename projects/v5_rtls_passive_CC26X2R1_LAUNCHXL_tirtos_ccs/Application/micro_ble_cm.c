...
uint8_t ubCM_findNextPriorityEvt(void)
{
  ...
  {
    ubCMConnInfo.ArrayOfConnInfo[cmConnSortedList->sessionId - 1].lastStartTime = ubCMConnInfo.ArrayOfConnInfo[cmConnSortedList->sessionId - 1].nextStartTime;
  ...
  if ((earliestSessionId == selectedSessionId) || (ubCM_isThereACollisionBetweenConn(earliestSessionId,selectedSessionId) == TRUE))
  {
    ...
    ubCMConnInfo.ArrayOfConnInfo[selectedSessionId - 1].lastStartTime = ubCMConnInfo.ArrayOfConnInfo[selectedSessionId - 1].nextStartTime;
  ...
  {
    ubCMConnInfo.ArrayOfConnInfo[earliestSessionId - 1].lastStartTime = ubCMConnInfo.ArrayOfConnInfo[earliestSessionId - 1].nextStartTime;
...
void ubCM_setupNextCMEvent(uint8_t sessionId)
{
  ...
  deltaTime = uble_timeDelta(currentTime, connInfo->timeStampMaster);
  ...
  // we got an indication from the master.
                                 ...
                                 uint8_t len, uint8_t *pPayload)
{
  ...
  if (newSessionActive == TRUE &&
      cmMaster == FALSE &&
      (uble_timeDelta(currentTime, connInfo->timeStampMaster) > (CM_CONN_INTERVAL_MIN * BLE_TO_RAT)))
  {
    connInfo->timeStampMaster2 = connInfo->timeStampMaster;
...
static void monitor_completeCB(bStatus_t status, uint8_t sessionId)
{
  ...
  if(connInfo->timeStampMaster2 != 0 && connInfo->consecutiveTimesMissed > 2)
  {
    connInfo->timeStampMaster  = connInfo->timeStampMaster2;
    connInfo->timeStampMaster2 = 0;
  }
  ...
  if (newSessionActive)
  {
    if (newSessionId == sessionId)
    {
      ...
      while (cmNewConnPend != NULL && newSessionId == CM_INVALID_SESSION_ID)
      {
        ...
        newSessionId = ubCM_startExt(elem->connHandle,
                                     elem->accessAddr,
                                     elem->connInterval,
                                     elem->hopValue,
                                     elem->currChan,
 ...
 */
uint8_t ubCM_startNewSession(uint8_t hostConnHandle, uint32_t accessAddress, uint16_t connInterval,
                             uint8_t hopValue, uint8_t currChan, uint8_t *chanMap, uint32_t crcInit)
{
  ...
  else
  {
    ...
    return (ubCM_startExt(hostConnHandle, accessAddress, connInterval, hopValue, currChan, chanMap, crcInit, reqTime));
  }
}
 ...
 */
uint8_t ubCM_startExt(uint8_t hostConnHandle, uint32_t accessAddress, uint16_t connInterval,
                      uint8_t hopValue, uint8_t nextChan, uint8_t *chanMap, uint32_t crcInit, uint32_t reqTime)
{
  ...
  if (ubCMConnInfo.numHandles <= CM_MAX_SESSIONS)
  {
    ...
    ubCMConnInfo.ArrayOfConnInfo[i].lastStartTime = ubCMConnInfo.ArrayOfConnInfo[i].currentStartTime;
