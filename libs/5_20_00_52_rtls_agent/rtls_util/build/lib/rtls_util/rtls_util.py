...
class RtlsUtil():
    ...
    def _log_incoming_msg(self, item, identifier):
        ...
        if sending_node in self._passive_nodes:
            ...
            self.logger.info(f"COORDINATOR  : {identifier} --> {new_dict}")
            ...
            try:
                ...
                if msg.command == "RTLS_CMD_CONNECT" and msg.type == "AsyncReq" and msg.payload.status == "RTLS_LINK_TERMINATED":
                    ...
                    if sending_node.identifier == self._coordinator_node.identifier:
                        ...
                        responder = self._get_responder_by_conn_handle(
    ...
    def set_devices(self, devices_setting):
        ...
        self.logger.info("RTLS Manager started")
        time.sleep(2)
        self._coordinator_node, self._passive_nodes, failed = self._rtls_manager.wait_identified()
        ...
        return self._coordinator_node, self._passive_nodes, self._all_nodes
    ...
    def scan(self, scan_time_sec, expected_responder_bd_addr=None, advSID=None):
        self._scan_results = []
        ...
        while not timeout_reached:
            ...
            self._coordinator_node.rtls.scan()
    ...
    def ble_connect(self, responder, connect_interval_mSec):
        if isinstance(responder, str):
            responder = self._get_responder_by_addr(responder)
            if responder is None:
                raise RtlsUtilScanResponderNotFoundException("Expected responder not found in scan list")
        ...
        self._conn_handle = None
        self._responder_attempt_to_connect = responder
        self._coordinator_node.connection_in_progress = True
        self._coordinator_node.connected = False

        self._coordinator_node.rtls.connect(
            responder['addrType'], responder['addr'], interval)
    ...
    def set_connection_interval(self, connect_interval_mSec, conn_handle=None):
        ...
        if str(self._coordinator_node.devId) == "DeviceFamily_ID_CC26X0R2":
            self._coordinator_node.rtls.set_rtls_param(
